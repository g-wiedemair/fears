...
static bool private_symbols_loaded() {
...
static void load_symbols() {
  /* If this is a developer station and the private pdb is already loaded leave it */
  if (private_symbols_loaded()) {
    return;
  }
...
static const char *windows_get_exception_description(const DWORD exceptioncode) {
...
static void windows_get_module_name(LPVOID address, PCHAR buffer, size_t size) {
  HMODULE mod;
  buffer[0] = 0;
...
static void windows_system_backtrace_exception_record(FILE *fp, PEXCEPTION_RECORD record) {
  char module[MAX_PATH];
  fprintf(fp, "Exception Record:\n\n");
  fprintf(fp,
          "ExceptionCode         : %s (0x%.8x)\n",
          windows_get_exception_description(record->ExceptionCode),
          record->ExceptionCode);
  fprintf(fp, "Exception Address     : 0x%p\n", record->ExceptionAddress);
  windows_get_module_name(record->ExceptionAddress, module, sizeof(module));
  ...
  if (record->ExceptionRecord) {
    fprintf(fp, "Nested ");
    windows_system_backtrace_exception_record(fp, record->ExceptionRecord);
  }
  fprintf(fp, "\n\n");
}

static bool windows_system_backtrace_run_trace(FILE *fp, HANDLE hThread, PCONTEXT context) {
  const int max_symbol_length = 100;

  bool result = true;

  PSYMBOL_INFO symbolinfo = static_cast<PSYMBOL_INFO>(
      mem_calloc(sizeof(SYMBOL_INFO) + max_symbol_length * sizeof(char), "crash Symbol table"));
  ...
  while (true) {
                    ...
                    0))
    {
      if (frame.AddrPC.Offset) {
        ...
        windows_get_module_name((LPVOID)frame.AddrPC.Offset, module, sizeof(module));
...
static bool windows_system_backtrace_stack_thread(FILE *fp, HANDLE hThread) {
  CONTEXT context = {0};
  ...
  return windows_system_backtrace_run_trace(fp, hThread, &context);
}

static bool windows_system_backtrace_stack(FILE *fp, const EXCEPTION_POINTERS *exception_info) {
  ...
  if (exception_info && exception_info->ExceptionRecord->ExceptionAddress) {
    ...
    return windows_system_backtrace_run_trace(fp, GetCurrentThread(), &TempContext);
  } else {
    /* If there is no current exception or the address is not set, walk the current stack. */
    return windows_system_backtrace_stack_thread(fp, GetCurrentThread());
  }
}

static void windows_system_backtrace_threads(FILE *fp) {
  fprintf(fp, "Threads:\n");
  ...
  do {
    if (te32.th32OwnerProcessID == GetCurrentProcessId()) {
      if (GetCurrentThreadId() != te32.th32ThreadID) {
        ...
        windows_system_backtrace_stack_thread(fp, ht);
        CloseHandle(ht);
      }
    }
...
static void windows_get_module_version(const char *file, char *buffer, size_t buffersize) {
  buffer[0] = 0;
  DWORD verHandle = 0;
  UINT size = 0;
...
static void windows_system_backtrace_modules(FILE *fp) {
  ...
  do {
    if (me32.th32ProcessID == GetCurrentProcessId()) {
      ...
      windows_get_module_version(me32.szExePath, version, sizeof(version));
...
void system_backtrace_with_os_info(FILE *fp, const void *os_info) {
  ...
  load_symbols();
  if (exception_info) {
    windows_system_backtrace_exception_record(fp, exception_info->ExceptionRecord);
  }
  if (windows_system_backtrace_stack(fp, exception_info)) {
    /* When the feap symbols are missing the stack traces will be unreliable
     * so only run if the previous step completed successfully. */
    windows_system_backtrace_threads(fp);
  }
  windows_system_backtrace_modules(fp);
}
