...
/**
 * When a thread allocated this amount of memory, the peak memory usage is updated.
 * An alternative would be to update the global peak memory after every allocation, but would cause
 * much overhead with little benefit.
 */
...
/** Check if the current memory usage is higher than the peak and update it */
static void update_global_peak() {
  Global &global = get_global();

  global.peak = std::max<usize>(global.peak, memory_usage_current());
