...
> TarjanScc<'graph, N, S, A, Neighbors>
{
    ...
    fn next_scc(&mut self) -> Option<&[N]> {
        ...
        if let (Some(start), Some(index_adjustment)) = (self.start.take(), self.index_adjustment.take()) {
            todo!()
        }
    ...
    fn visit_once(&mut self, v: N, mut v_is_local_root: bool) -> Option<usize> {
        ...
        let start = self
            ...
            .iter()
            .rposition(|&w| {
                if nodes[self.graph.to_index(v)].root_index > nodes[self.graph.to_index(w)].root_index {
                    true
                } else {
