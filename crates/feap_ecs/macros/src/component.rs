...
use syn::{
    braced, parenthesized, parse::Parse, parse_macro_input, parse_quote, punctuated::Punctuated, spanned::Spanned, token::{Brace, Comma, Paren}, Data, DataEnum,
    DataStruct, DeriveInput, Expr, ExprCall, ExprPath, Field, Fields, Ident, LitStr,
    Member,
    Path, Result,
    Token,
    Type,
...
pub fn derive_component(input: TokenStream) -> TokenStream {
    ...
    let relationship = match derive_relationship(&ast, &attrs, &feap_ecs_path) {
        Ok(value) => value,
        Err(err) => err.into_compile_error().into(),
    };
    let relationship_target = match derive_relationship_target(&ast, &attrs, &feap_ecs_path) {
        Ok(value) => value,
        Err(err) => err.into_compile_error().into(),
    };
    ...
    let map_entities = map_entities(
        ...
        fn map_entities<M: #feap_ecs_path::entity::EntityMapper>(this: &mut Self, mapper: &mut M) {
            use #feap_ecs_path::entity::MapEntities;
            #map_entities_impl
        }
    });

    let storage = storage_path(&feap_ecs_path, attrs.storage);

    let on_add_path = attrs
        .on_add
        .map(|path| path.to_token_stream(&feap_ecs_path));
    ...
    let on_insert_path = if relationship.is_some() {
        ...
        Some(quote!(<Self as #feap_ecs_path::relationship::Relationship>::on_insert))
    } else {
        attrs
            .on_insert
            .map(|path| path.to_token_stream(&feap_ecs_path))
    ...
    let on_replace_path = if relationship.is_some() {
        ...
        Some(quote!(<Self as #feap_ecs_path::relationship::Relationship>::on_replace))
        ...
        Some(quote!(<Self as #feap_ecs_path::relationship::RelationshipTarget>::on_replace))
    } else {
        attrs
            .on_replace
            .map(|path| path.to_token_stream(&feap_ecs_path))
        ...
        .is_some_and(|target| target.linked_spawn)
    {
        ...
        Some(quote!(<Self as #feap_ecs_path::relationship::RelationshipTarget>::on_despawn))
    } else {
        attrs
            .on_despawn
            .map(|path| path.to_token_stream(&feap_ecs_path))
    ...
    let on_add = hook_register_function_call(&feap_ecs_path, quote! {on_add}, on_add_path);
    let on_insert = hook_register_function_call(&feap_ecs_path, quote! {on_insert}, on_insert_path);
    let on_replace =
        hook_register_function_call(&feap_ecs_path, quote! {on_replace}, on_replace_path);
    let on_remove = hook_register_function_call(&feap_ecs_path, quote! {on_remove}, on_remove_path);
    let on_despawn =
        hook_register_function_call(&feap_ecs_path, quote! {on_despawn}, on_despawn_path);
    ...
    let mutable_type = (attrs.immutable || relationship.is_some())
        .then_some(quote! { #feap_ecs_path::component::Immutable })
        .unwrap_or(quote! { #feap_ecs_path::component::Mutable });

    let clone_behavior = if relationship_target.is_some() || relationship.is_some() {
        quote!(
            use #feap_ecs_path::relationship::{
            ...
            (&&&&&&&#feap_ecs_path::relationship::RelationshipCloneBehaviorSpecialization::<Self>::default()).default_clone_behavior()
        )
    } else if let Some(behavior) = attrs.clone_behavior {
        quote!(#feap_ecs_path::component::ComponentCloneBehavior::#behavior)
    } else {
        quote!(
            use #feap_ecs_path::component::{DefaultCloneBehaviorBase, DefaultCloneBehaviorViaClone};
            (&&&#feap_ecs_path::component::DefaultCloneBehaviorSpecialization::<Self>::default()).default_clone_behavior()
        )
    };
    ...
    TokenStream::from(quote! {
        ...
        impl #impl_generics #feap_ecs_path::component::Component for #struct_name #type_generics #where_clause {
            const STORAGE_TYPE: #feap_ecs_path::component::StorageType = #storage;
            type Mutability = #mutable_type;
            fn register_required_components(
                _requiree: #feap_ecs_path::component::ComponentId,
                required_components: &mut #feap_ecs_path::component::RequiredComponentsRegistrator,
            ) {
                #(#register_required)*
            }
            ...
            fn clone_behavior() -> #feap_ecs_path::component::ComponentCloneBehavior {
                #clone_behavior
            }
...
pub(crate) fn map_entities(
    ...
    if let Some(map_entities_override) = map_entities_attr {
        let map_entities_tokens = map_entities_override.to_token_stream(feap_ecs_path);
        return Some(quote!(
...
impl HookAttributeKind {
    ...
    fn to_token_stream(&self, feap_ecs_path: &Path) -> TokenStream2 {
        match self {
            HookAttributeKind::Path(path) => path.to_token_stream(),
            HookAttributeKind::Call(call) => {
                quote!({
                    fn _internal_hook(world: #feap_ecs_path::world::DeferredWorld, ctx: #feap_ecs_path::lifecycle::HookContext) {
                        (#call)(world, ctx)
                    }
                    _internal_hook
                })
    ...
    fn to_token_stream(&self, feap_ecs_path: &Path) -> TokenStream2 {
        match self {
            ...
            MapEntitiesAttributeKind::Default => {
                quote!(
                   <Self as #feap_ecs_path::entity::MapEntities>::map_entities
                )
            }
        }
    }
...
fn storage_path(feap_ecs_path: &Path, ty: StorageTy) -> TokenStream2 {
    let storage_type = match ty {
        StorageTy::Table => Ident::new("Table", Span::call_site()),
        StorageTy::SparseSet => Ident::new("SparseSet", Span::call_site()),
    };

    quote! { #feap_ecs_path::component::StorageType::#storage_type }
}
...
fn hook_register_function_call(
    ...
    function.map(|meta| {
        quote! {
            fn #hook() -> ::core::option::Option<#feap_ecs_path::lifecycle::ComponentHook> {
...
fn derive_relationship(
    ...
    Ok(Some(quote! {
        impl #impl_generics #feap_ecs_path::relationship::Relationship for #struct_name #type_generics #where_clause {
            type RelationshipTarget = #relationship_target;

            #[inline(always)]
            fn get(&self) -> #feap_ecs_path::entity::Entity {
                self.#relationship_member
            }

            #[inline]
            fn from(entity: #feap_ecs_path::entity::Entity) -> Self {
                Self {
...
fn derive_relationship_target(
    ...
    Ok(Some(quote! {
        impl #impl_generics #feap_ecs_path::relationship::RelationshipTarget for #struct_name #type_generics #where_clause {
